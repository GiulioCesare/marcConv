...
//    bytesRead = input.read(byteArray);
  	  ...
  	  // Gestiamo anche i record con Newline!! 26/03/2014
  	  retb = in->Read(buf, 2);
  	  if (retb)
  	  {
 		  if (buf[0] == '\r' &&  buf[1] == '\n') // 0d/0a
 		  ...
 		  {
 			buf[0] = buf[1];
   		    retb = in->Read(buf+1, sizeof(buf)-2);
 		  }
 		  else
  		    retb = in->Read(buf+2, sizeof(buf)-3);
...
//      printf ("\n\nTAG = %s", de->tag.data());
//      printf ("\nLENGTH = %d", de->length);
//      printf ("\nSTART = %d (hex %x) absolute %d, hex=%x", de->start, de->start, de->start + record->getLeader()->getBaseAddressOfData(), de->start + record->getLeader()->getBaseAddressOfData() );
...
//      throw new MarcException("expected record terminator");
    ...
    {
    	fprintf (stderr, "expected record terminator");
      return 0;
    }
...
//  while (bytesRead < MAX_SUBFIELD_LENGTH) {
  ...
  fprintf (stderr, "subfield not terminated");
